"""普通 Verilator CSV 适配器，保留独立构建、初始化及采样证据。"""
...
def _driver(inputs: list[dict], outputs: list[dict], clock: str, count: int) -> str:
    ...
    return (
        """// 自动生成：每行低电平稳定后读取输出；最后一行之后不提交上升沿。
#include "Vcsvdut.h"
#include "verilated.h"
#include <algorithm>
#include <cstdint>
...
def run_native(
    ...
    contract = {
        ...
        "clock": clock,
        "events": "每行 drive(clk=0, inputs) -> eval -> 读取；除最后一行外 drive(clk=1) -> eval",
        ...
        "time": "每次事件递增 1 个模拟精度单位；本适配器面向无延迟 DUT 的边沿驱动",
    ...
    arguments = [
        ...
        "sim",
        "--timescale",
        str(manifest.get("timescale", "1ns/1ps")),
    ]
