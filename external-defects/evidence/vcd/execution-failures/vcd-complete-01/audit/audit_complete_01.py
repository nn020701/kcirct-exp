...
BATCH = ROOT / '.runs/vcd-complete-01'
OUT = ROOT / '.work/complete-exp/vcd-complete-01-audit.json'
errors=[]
...
check(collections.Counter(x['status'] for x in results)=={'pass':8,'execution_error':2},'历史状态应为8通过2执行错误')
...
check(exit_record['host_exit_code']==2 and exit_record['runner_exit_code']==1 and exit_record['status']=='completed_with_failures','批次宿主/runner退出记录不符')
file_records=[]
