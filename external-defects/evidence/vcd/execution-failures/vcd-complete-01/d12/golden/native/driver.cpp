...
int main(int argc, char **argv) {
  ...
  output << "s_axis_tready,m_axis_tdata,m_axis_tkeep,m_axis_tvalid,m_axis_tlast,m_axis_tid,m_axis_tdest,m_axis_tuser,status_overflow,status_bad_frame,status_good_frame\n";
  ...
  for (size_t sample = 0; sample < 16; ++sample) {
    ...
    output << decimal({uint32_t(uint64_t(dut.status_overflow) >> 0)}) << ",";
    output << decimal({uint32_t(uint64_t(dut.status_bad_frame) >> 0)}) << ",";
    output << decimal({uint32_t(uint64_t(dut.status_good_frame) >> 0)}) << "\n";
    if (sample + 1 < 16) {
      context.timeInc(1);
      dut.clk = 1;
      dut.eval();
