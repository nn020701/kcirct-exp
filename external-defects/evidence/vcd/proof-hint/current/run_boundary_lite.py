...
DEFINITION = WORK / "definition-lite"
...
def main():
    assert json.loads((WORK / "build-lite.json").read_text())["returncode"] == 0
    ...
    for depth, hint in ((1023, False), (1024, False), (1024, True)):
        name = f"lite-depth-{depth}" + (".hint" if hint else ".kore")
        output = WORK / name
        ...
        (WORK / "runs-lite.json").write_text(json.dumps(records, indent=2) + "\n")
