...
int main(int argc, char **argv) {
  ...
  output << "input_axis_tready,output_axis_tdata,output_axis_tkeep,output_axis_tvalid,output_axis_tlast,output_axis_tuser\n";

  for (size_t sample = 0; sample < 13; ++sample) {
    dut.clk = 0;
    uint64_t value = 0;
    if (!(stimulus >> std::hex >> value)) return 3; dut.rst = value;
    if (!(stimulus >> std::hex >> value)) return 3; dut.input_axis_tdata = value;
    if (!(stimulus >> std::hex >> value)) return 3; dut.input_axis_tkeep = value;
