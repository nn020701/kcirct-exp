...
int main(int argc, char **argv) {
  ...
  output << "frame_len,frame_len_valid\n";

  for (size_t sample = 0; sample < 6; ++sample) {
    dut.clk = 0;
    uint64_t value = 0;
    if (!(stimulus >> std::hex >> value)) return 3; dut.rst = value;
    if (!(stimulus >> std::hex >> value)) return 3; dut.monitor_axis_tkeep = value;
    if (!(stimulus >> std::hex >> value)) return 3; dut.monitor_axis_tvalid = value;
    if (!(stimulus >> std::hex >> value)) return 3; dut.monitor_axis_tready = value;
    if (!(stimulus >> std::hex >> value)) return 3; dut.monitor_axis_tlast = value;
    dut.eval();
    if (context.gotFinish()) return 4;
    trace.dump(context.time());
    output << decimal({uint32_t(uint64_t(dut.frame_len) >> 0)}) << ",";
    output << decimal({uint32_t(uint64_t(dut.frame_len_valid) >> 0)}) << "\n";
    if (sample + 1 < 6) {
      context.timeInc(1);
      dut.clk = 1;
      dut.eval();
