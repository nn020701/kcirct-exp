...
def prepare(args: argparse.Namespace) -> None:
    ...
    versions.update({"definition_sha256": sha(definition / "definition.kore"), "parser_sha256": sha(BUILD / "parser")})
...
def baseline(manifest: dict, variant: str) -> tuple[Path, dict]:
    ...
    folder = ROOT / "designs" / manifest["project_id"] / "mlir" / manifest["variant_id"] / variant
...
def run(args: argparse.Namespace) -> None:
    ...
    for identifier in selected:
        ...
        try:
            ...
            r["buggy_k_native"] = r["waveform_comparisons"].get("buggy", {}).get("agrees")
            r["simulator_agrees_with_native"] = all(
                r["waveform_comparisons"].get(v, {}).get("agrees") is True for v in ("golden", "buggy")
            )
            ...
            if all(
                ...
                r["status"] = (
                    failures[0]
                    if failures
                    else "vcd_mismatch" if not r["simulator_agrees_with_native"] else "oracle_mismatch"
                )
        except Exception as error:
